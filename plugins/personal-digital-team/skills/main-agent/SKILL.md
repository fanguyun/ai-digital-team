...
1. 先理解目标、交付物、时限、可用资料和潜在外部影响，再决定是否路由。
   - 需要确认当前插件包含哪些岗位时，可只读执行 `node ../$scriptslist-team.mjs --format json`；脚本结果仅作路由辅助，不替代对岗位规则的完整阅读。
   - 用户报告插件安装异常、岗位缺失或元数据显示不全时，可只读执行 `node ../$scriptscheck-installation.mjs`，并根据输出说明缺失项；不得用该脚本修改、安装或同步插件。
